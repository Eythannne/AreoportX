...
$pilotes = (new RepositoryPilote())->listePilote();
$avions = (new RepositoryAvion())->listeAvion();

$RepositoryVol = new RepositoryVol();
$vol = $RepositoryVol->detailVol($_GET["id"]);
$idVol = $_GET['id'];
?>
...
<div class="container mt-4">
    <h1>Modification vol</h1>
    <?= var_dump($vol)?>
    <form action="../src/traitement/vol/traitementModificationVol.php" method="post">
        <div class="mb-3">
            <label for="destination" class="form-label">Déstination</label>
            <input type="text" class="form-control" id="destination" name="destination" value="<?=$vol->getDestination()?>">
        </div>
        <div class="mb-3">
            <label for="date" class="form-label">Date</label>
            <input type="date" class="form-control" id="date" name="date" value="<?=$vol->getDate()?>">
        </div>
        <div class="mb-3">
            <label for="heure_depart" class="form-label">Heure de depart</label>
            <input type="time" class="form-control" id="heure_depart" name="heure_depart" value="<?= substr($vol->getHeureDepart(), 0, 5) ?>">        </div>
        <div class="mb-3">
            <label for="heure_arrivee" class="form-label">heure d'arrivée</label>
            <input type="time" class="form-control" id="heure_arrivee" name="heure_arrivee" value="<?= substr($vol->getHeureArrivee(), 0, 5) ?>">        </div>
        <div class="mb-3">
        ...
        <div class="mb-3">
            ...
            <select class="form-control" id="ref_pilote" name="ref_pilote">
                <?php foreach ($pilotes as $pilote): ?>
                    <option value="<?= $pilote['id_pilote'] ?>" <?= $vol->getRefPilote() == $pilote['id_pilote'] ? 'selected' : '' ?>>
